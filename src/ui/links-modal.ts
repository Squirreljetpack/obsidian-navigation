import { type App, type MarkdownView, Platform, SuggestModal, type ViewState, type WorkspaceLeaf } from "obsidian";
import { extractLinksFromView, type LinkItem, parseLinksFromText } from "../utils/link-parser.js";
...
export class LinksModal extends SuggestModal<LinkItem> {
  ...
  private originalLeafEState: unknown = null;
  ...
  onOpen(): void {
    ...
    // Defaults are added by the parent's onOpen.
    // Modifier combinations route through onChooseSuggestion,
    // we only need to register `Mod+S`.
    this.registerKeybindings();

    // Tack on preview behavior
    if (this.chooser) {
  ...
  private findPreviewElement(
    ...
    if (textKey) {
      ...
      if (textMatches.length > 0) {
        const textOccurrence = this.items
          ...
          .indexOf(item);
        const index = textOccurrence >= 0 && textOccurrence < textMatches.length
          ? textOccurrence
          : 0;
    ...
    if (idxEl) {
      const raw = idxEl.getAttribute("data-href")
