...
import {
	DEFAULT_SETTINGS,
	type FolderNavigatorSettings,
	FolderNavigatorSettingTab,
} from "./settings.js";
...
export default class FolderNavigatorPlugin extends Plugin {
	...
	async onload() {
		...
		const openModal = () => {
			const initialItem =
				this.app.workspace.getActiveFile() ?? this.app.vault.getRoot();
	...
	async loadSettings() {
		this.settings = Object.assign(
			{},
			DEFAULT_SETTINGS,
			(await this.loadData()) as Partial<FolderNavigatorSettings>,
		);
