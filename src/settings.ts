...
export class FolderNavigatorSettingTab extends PluginSettingTab {
	...
	display(): void {
		...
		new Setting(containerEl)
			...
			.setDesc(
				"Select the default sort order for files and folders in the navigator modal.",
			)
		...
		new Setting(containerEl)
			...
			.setDesc(
				"Colon-separated directory paths appended to path on macOS (e.g. /usr/local/bin:/opt/homebrew/bin).",
			)
			.addText((text) =>
				text
		...
		new Setting(containerEl)
			...
			.setDesc(
				"Semicolon-separated directory paths appended to path on Windows.",
			)
			.addText((text) =>
				text
		...
		new Setting(containerEl)
			...
			.setDesc(
				"Colon-separated directory paths appended to path on Linux (e.g. /usr/local/bin:~/.local/bin).",
			)
			.addText((text) =>
				text
		...
		new Setting(containerEl)
			...
			.setDesc(
				"Define hotkey mappings and program command templates. Use {} for the target file or folder path.",
			)
			.addButton((button) =>
				button
